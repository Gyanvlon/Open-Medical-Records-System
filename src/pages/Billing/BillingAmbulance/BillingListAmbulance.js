...
function BillingListAmbulance(props) {
  const classes = useStyles();
  const ref = useRef();
  const { id } = props?.popupListData;
  const [addAmbulancePopup, setAddAmbulancePopup] = React.useState(false);
  ...
  if (addAmbulancePopup) {
    ...
    return (
      <>
        ...
        {open && <AmbulanceBillsView setOpen={setOpen} open={open} biilingtableinfo={biilingtableinfo}/> }
      </>
    );
  }
}

export default BillingListAmbulance;
