...
function Ndhmlayout(props) {
  ...
  console.log('txnId Ndhmlayout:', txnId)
  const handleClose = () => {
    setOpen(false);
  };
  ...
  const handleOTPlayout = async (event) => {
    ...
    if (patientData && patientData?.data?.length > 0) {
      ...
      var { data } = await generateAadhaarOtp(aadharvalue, setLoader);
  ...
  // hande abha umber verify button
  useEffect(() => {
    if (phrid.length > 0) setIsVerifyId(false)
    else setIsVerifyId(true)
  }, [phrid])
  ...
  const handlePhridautentication = async (e) => {
    ...
    if (patientData && patientData?.data?.length > 0) {
      ...
      const { data } = await searchByHealthId(phrid, setLoader);

      if (!data) {
        setLoader(false);
        swal({
          title: "Oops!",
          text: "No user account can be found with the ABHA Number/Enrolment Number provided!",
          icon: "error",
        })
        return
      }
      ...
      if (data) {
        setLoader(true);
        const res2 = await initiateAuth(data?.healthId, setLoader);
        ...
        if (!res2?.data) {
          ...
          swal({
            title: "Warning",
            text: "Please wait for 30 minutes before sending another OTP request!",
            icon: "warning",
          })
          return
        }
  ...
  const confirmAddharOTP = async () => {
    ...
    if (res?.data) {
      setLoader(true);
      var { data } = await getProfile(res?.data?.token, "application/json;charset=UTF-8");
          ...
          <Grid container spacing={3} className={classes.customMargin}>
            ...
            <Grid item sm={5} xs={12}>
              <TextField
                ...
                onChange={(e) => setphrid(e.target?.value)}
              />
            </Grid>
