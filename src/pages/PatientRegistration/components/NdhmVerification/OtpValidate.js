...
function OtpValidate(props) {
  ...
  const handlefrmSaubmit = async (e) => {
    ...
    if (data) {
      ...
      if (data.healthIdNumber != null) {
        ...
        if (data?.gender === "M") {
          formValues["Gender*"] = { name: "Male", value: data?.genderr }
        }
        ...
        setFormValues({
          ...
          "Last Name": lastName,
          "Date of Birth": new Date(moment(data?.birthdate).format("DD/MM/yyyy")),
  ...
  const handlemobilevalidateOtp = async (e) => {
    ...
    if (formValues["Phone Number*"] !== "") {
      ...
      console.log('mobile, txnId', mobile, txnId)
      var res = await generateMobileOTP(mobile, txnId, setLoader);
      if (res) {
        setLoader(false);
