...
function CreateAbhaNumber(props) {
  ...
  const handlefrmSaubmit = async(e) => {
    setLoader(true);
    var res = await verifyAadhaarOtp(verifiedTxnId,otpvalue,setLoader);
    if(res){
  ...
  const handlemobilevalidateOtp = async(e) => {
    setLoader(true);
    if (mobile !== "") {
        var res = await generateMobileOTP(mobile.substring(2),verifiedTxnId,setLoader);
        if(res){
          setLoader(false);
          setverifiedTxnId(res.txnId);
  ...
  if (mobileotpvalue === true) {
    return (
        ...
        aadhaar={props.aadhaar} txnId = {verifiedTxnId} 
