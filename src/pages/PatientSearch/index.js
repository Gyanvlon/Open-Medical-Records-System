...
import "./styles.css";
import { PRE_NUM } from "../../utils/constants";
import SearchWithQRCode from "./SearchWithQRCode";
...
export default function PatientSearch(props) {
  ...
  var [healthNo, setHealthNo] = useState("");
  ...
  var [HealthErrorMsj, setHealthErrorMsj] = useState();
  ...
  const isEnteredPressed = (charLen, event, name, eventName) => {
    if (
      ...
      if (name == "firstName") {
        ...
        } else if (errors.healthIdData) {
          setNameErrorMsj("Health Id is required");
        }
      }
  ...
  const isSearchClicked = (charLen, event, name, eventName) => {
    if (eventName == "clicked") {
      if (
        ...
        (healthId && healthId.length > charLen)
      ) {
        return true;
      } else {
        var nameLen = "";
        ...
        setErrors({ phoneData: !phone ? false : true });
        setErrors({ identifierData: !identifier ? false : true });
        setErrors({ healthIdData: !healthId ? false : true });
        if (firstName) {
        ...
        if (healthId) {
          healthLen = healthId.length;
        }
        if (healthLen && nameLen <= charLen && nameLen > 0) {
          setHealthErrorMsj("Atleast 3 characters is required");
        } else if (errors.nameData) {
          setHealthErrorMsj("ABHA Address is required");
        }
  ...
  function checkData(
    ...
    if (healthId) {
      if (healthId && ageVal) {
        param = healthId + "&agerange=" + ageRange + "&age=" + ageVal;
      } else if (healthId && lvdValueApprox) {
        param = healthId + +paramLvd;
      } else if (healthId && lvd) {
        param = healthId + "&lastvisitapprox=" + lvd;
      } else {
        param = healthId;
      }
    }
  ...
  function validateAge(name, value) {
    ...
    if (!regex.test(value)) {
      ...
      setFormErrors({
        ...formErrors,
        [age]: "Please enter age in given format (20y /20m/ 20d)",
      });
    } else {
  ...
  /* const abhaValidation = (name,value) =>{
    const HEALTHID = /^\d{2}-?\d{4}-?\d{4}-?\d{4}$/;
    const EMAIL =/^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    const healthIDTest =   HEALTHID.test(value);
    const healthAddressTest =   EMAIL.test(value);
    if(!healthIDTest || !healthAddressTest){
      setHealthErrorMsj(true);
      setFormErrors({ ...formErrors, [name]: "Please enter valid abha address/ number" });
    }
    else {
      setFormErrors({ ...formErrors, [name]: "" });
    }
  ...
  const searchOnKey = (event, name, eventName) => {
    var searchValue = "";
    if (healthNo !== "") {
      searchValue = healthNo;
      //abhaValidation(name,healthNo);
    } else {
      searchValue = event.target.value;
      //abhaValidation(name,event.target.value);
    }
    ...
    if (name == "healthId") {
      setHealthId(searchValue);
      //abhaValidation(name,searchValue);
    }
    if (healthNo !== "") {
      setHealthId(healthNo);
    }
    if (name == "identifier") {
    ...
    if (isValueEntered(event, name, eventName)) {
      ...
      if (isDataAlready) {
        ...
        if (healthId) {
          filters["healthId"] = [healthId.toUpperCase()];
        }
        if (phone) {
        ...
        if (alreadystoredata.length > 0) {
          ...
          if (
            ...
            const url = `/search/patients?name=${param}`;
            getaddressAPI(url)
              .then((response) => {
                setapihit(true);
                var phoneNo = "";
                ...
                for (let i = 0; i < response.data.length; i++) {
                  ...
                  let nameval =
                    searchdatanew[0][i]["name"].indexOf(",") > -1
                      ? searchdatanew[0][i]["name"].split(",")[0].toUpperCase()
                      : searchdatanew[0][i]["name"].toUpperCase();
                  ...
                  if (searchdatanew[0][i]["person_attributes"]) {
                    ...
                    healthId =
                      searchdatanew[0][i]["person_attributes"]["Health Number"];
                  }
                ...
                try {
                  if (storedata.length > 0) {
                    ...
                    if (healthIdValue) {
                      filters["healthId"] = [healthIdValue];
                    }
                    if (lvdValue) {
                      filters["lvd"] = [lvdValue];
                    }
        ...
        if (firstNameValue || phoneValue || identifierValue || healthIdValue) {
          setnameData(false);
          setphoneData(false);
  ...
  function SimpleDialog(props) {
    ...
    const generateQRCodeResponse = async (data) => {
      setHealthId(data.hidn);
      setScannerOpen(false);
      //searchOnKey({ searchDetails }, "clicked");
    };
    return (
      <Dialog
        fullWidth
        ...
        open={open}
      >
        <DialogTitle id="simple-dialog-title">Scan ABHA QR</DialogTitle>
        <DialogContent dividers>
          <SearchWithQRCode generateQRCodeResponse={generateQRCodeResponse} />
        </DialogContent>
        <DialogActions>
              ...
              <TextField
                error={
                  (!errors.phoneData &&
                    ...
                    !errors.identifierData &&
                    !errors.healthIdData) ||
                  (formErrors["healthId"] ? true : false)
                }
                helperText={
                  (!errors.phoneData &&
                    !errors.nameData &&
                    ...
                    !errors.healthIdData &&
                    HealthErrorMsj) ||
                  (errors.nameData && HealthErrorMsj) ||
                  formErrors["healthId"]
                }
                name="healthId"
                variant="outlined"
                ...
                onKeyUp={(e) => searchOnKey(e, "healthId", "press")}
                className={classes.field}
              />
            </GridItem>
            <GridItem item xs={12} sm={6} md={3}>
            ...
            <GridItem item xs={12} sm={6} md={6}>
              <FormControl component="fieldset" fullWidth>
                ...
                >
                  <FormControlLabel
                    value="F"
                    control={<Radio style={{ paddingRight: "1px" }} />}
                    label="Female"
                  />
                  <FormControlLabel
                    value="M"
