...
import { deleteUnit, editableUnit } from "../../../../../actions/inventory"
const useStyles = makeStyles(styles);
export default function ManageUnitTab() {
  ...
  const isUnitDelete = useSelector(state => state.inventory.isUnitDelete)
  ...
  const handleDelete = (event) => {
   if(CbId.length>0){
    swal({
      title: "Are you sure?",
      text: "Once Delete Unit, You will not be able to recover this Unit!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((value) => {
          ...
          <div mt={2}>
            <Card >
             ...
             <Button variant="contained" color="primary"
                 ...
                 component={Link}
                 to="/app/system-administration/manage_add_unit">
                 Add Unit
                </Button>
              <CardContent>
          ...
          <Table aria-label="spanning table">
            ...
            {stableSort(isSearched ? rows : unitList, getComparator(order, orderBy))
                ...
                <TableCell component={Link}
                 to={`/app/system-administration/manage_update_unit?unitUuid=${ele.uuid}`} >{ele.name}</TableCell>
